...
extension BuildOperationTargetStarted: DecodableRPCPayload {
    public init(args: [MessagePackValue], indexPath: IndexPath) throws {
        ...
        targetID = try args.parseInt64(indexPath: indexPath + IndexPath(index: 0))
        guid = try args.parseString(indexPath: indexPath + IndexPath(index: 1))
        targetInfo = try args.parseObject(indexPath: indexPath + IndexPath(index: 2))
    }
}
