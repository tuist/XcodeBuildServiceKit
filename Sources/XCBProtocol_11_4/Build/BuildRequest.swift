...
extension BuildRequest: DecodableRPCPayload {
    public init(args: [MessagePackValue], indexPath: IndexPath) throws {
        guard args.count == 16 else { throw RPCPayloadDecodingError.invalidCount(args.count, indexPath: indexPath) }
        ...
        enableIndexBuildArena = try args.parseBool(indexPath: indexPath + IndexPath(index: 13))
        useLegacyBuildLocations = try args.parseBool(indexPath: indexPath + IndexPath(index: 14))
        shouldCollectMetrics = try args.parseBool(indexPath: indexPath + IndexPath(index: 15))
    }
}
