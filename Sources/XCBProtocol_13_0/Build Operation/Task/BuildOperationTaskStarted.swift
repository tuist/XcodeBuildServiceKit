...
public struct BuildOperationTaskStarted {
    public struct TaskDetails {
        ...
        public let ruleInfo: String // e.g. "CompileSwift normal x86_64
        // /Users/USER/Desktop/BazelXCBuildServer/Sources/XCBProtocol/Response.swift", "PhaseScriptExecution SwiftLint /Users/USER/Library/Developer/Xcode/DerivedData/PROJECT-HASH/Build/Intermediates.noindex/PROJECT.build/Debug-iphonesimulator/SCRIPT.build/Script-9A635388D017DF17C1E0081A.sh"
        public let executionDescription: String // e.g. "Compile
        // /Users/USER/Desktop/BazelXCBuildServer/Sources/XCBProtocol/Response.swift", "Run custom shell script 'SwiftLint'"
        public let commandLineDisplayString: String? // e.g. "    cd /Users/USER/Desktop/BazelXCBuildServer\n/Applications/Xcode-11.3.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/swift
        // -frontend -c ..."
...
extension BuildOperationTaskStarted: DecodableRPCPayload {
    public init(args: [MessagePackValue], indexPath: IndexPath) throws {
        ...
        targetID = try args.parseOptionalInt64(indexPath: indexPath + IndexPath(index: 1))
        parentTaskID = try args.parseOptionalInt64(indexPath: indexPath + IndexPath(index: 2))
        taskDetails = try args.parseObject(indexPath: indexPath + IndexPath(index: 3))
    }
}
...
extension BuildOperationTaskStarted.TaskDetails: DecodableRPCPayload {
    public init(args: [MessagePackValue], indexPath: IndexPath) throws {
        ...
        taskName = try args.parseString(indexPath: indexPath + IndexPath(index: 0))
        signature = try args.parseBinary(indexPath: indexPath + IndexPath(index: 1))
        ruleInfo = try args.parseString(indexPath: indexPath + IndexPath(index: 2))
        executionDescription = try args.parseString(indexPath: indexPath + IndexPath(index: 3))
        commandLineDisplayString = try args.parseOptionalString(indexPath: indexPath + IndexPath(index: 4))
        interestingPath = try args.parseOptionalString(indexPath: indexPath + IndexPath(index: 5))
        serializedDiagnosticsPaths = try args.parseStringArray(indexPath: indexPath + IndexPath(index: 6))
    }
}
